...
export class BorderStylePlugin extends StylePlugin {
    ...
    public async configToStyleRules(contract: BorderContract): Promise<StyleRule[]> {
        const result = [];
        ...
        if (contract.top && contract.top.width && contract.top.style && contract.top.colorKey) {
            ...
            result.push(new StyleRule("borderTop", `${StylePlugin.parseSize(contract.top.width)} ${contract.top.style} ${borderTopColorContract.value}`));
        }
        else {
        ...
        if (contract.left && contract.left.width && contract.left.style && contract.left.colorKey) {
            ...
            result.push(new StyleRule("borderLeft", `${StylePlugin.parseSize(contract.left.width)} ${contract.left.style} ${borderLeftColorContract.value}`));
        }
        else {
        ...
        if (contract.right && contract.right.width && contract.right.style && contract.right.colorKey) {
            ...
            result.push(new StyleRule("borderRight", `${StylePlugin.parseSize(contract.right.width)} ${contract.right.style} ${borderRightColorContract.value}`));
        }
        else {
        ...
        if (contract.bottom && contract.bottom.width && contract.bottom.style && contract.bottom.colorKey) {
            ...
            result.push(new StyleRule("borderBottom", `${StylePlugin.parseSize(contract.bottom.width)} ${contract.bottom.style}, ${borderBottomColorContract.value}`));
        }
        else {
